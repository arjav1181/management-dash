...
export default function HFError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <div className="min-h-[40vh] flex flex-col items-center justify-center text-center animate-fadeIn">
      <div className="w-16 h-16 rounded-2xl bg-rose/10 flex items-center justify-center mb-4">
        <Boxes size={32} className="text-rose" />
      </div>
      <h2 className="text-lg font-semibold text-text-primary">Failed to load HF Spaces</h2>
