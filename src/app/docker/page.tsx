...
export default function DockerPage() {
  ...
  const fetchRepos = async () => {
    if (!hasToken('docker')) { setLoading(false); return; }
    setLoading(true);
    try {
      const res = await fetch('/api/docker/repos');
  ...
  useEffect(() => { fetchRepos(); }, [hasToken('docker')]);
    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="flex items-center gap-4">
        <div className="relative flex-1 max-w-sm">
          ...
          <input aria-label="Search images" className="w-full rounded-lg border border-border-primary bg-bg-tertiary pl-10 pr-4 py-2.5 text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-accent/40" placeholder="Search images..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
